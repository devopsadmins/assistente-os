...
| **SPEC-HR3** | "todo handler MCP precisa de `authorizeTool` + evento em `audit-trail.ts`" | **Parcialmente feito.** `authorizeTool` já é um *blanket check* chamado antes do `switch` de todas as tools (`packages/tools/src/index.ts:103`) — a parte de autorização já existe. Mas `audit-trail.ts` é um export sem nenhum chamador real no repo (módulo morto). | **Reescrever como delta**: só falta ligar `audit-trail.ts` no dispatch de tools. Não é "todo handler" — é um módulo a conectar. |
...
| **SPEC-GR4** | Gate de merge: `build`+`test`+supervisor ≥ 95 no CI. Pré-requisito antes de qualquer refactor grande (Onda 3e) ou automação autônoma (ideia 11 do arquivo de ideias). | P1 | — |
| **DS14+DS15** | `StreamingText` sem sinal de "acabou" (cursor pisca pra sempre) + `MessageList`/`StreamingText` sem `aria-live`/`role="log"`. Mesma causa raiz (ciclo de vida do stream não expresso) — tratar como um pacote único, promover a gate antes de release público do ThreadScreen. | P2→gate | — |
...
| **M3** | Prompt injection — screening regex-only, não-bloqueante por default | P1 |
