...
|---|---|---|---|
| **SPEC-HR1** | 🟡 **Fatia 1 (2026-09-04) + Fatia 2 (2026-09-05).** Fatia 1: `POST /chat` degrada pra 200+Markdown via `isDbHealthy`+`handleChatDegraded` (ver detalhe histórico abaixo). **Achado da Fatia 2 (auditoria antes de codar, mesmo padrão da seção 0)**: `soul_decidir`/`soul_licao`/`soul_record_lesson`/`soul_get_lessons` **já eram 100% resilientes** à queda do Postgres — são puramente arquivo/JSONL (`decisoes/*.md`, `licoes.md`, `governance/incidents.jsonl`), sem nenhum `pool.query` no caminho de execução; só não tinham sido riscados do "escopo restante". O item genuíno que faltava era **agenda** (100% Postgres, zero fallback) — corrigido: `agenda_add`/`agenda_list` (tool MCP) agora sondam `isDbHealthy` antes da query e lançam mensagem clara ("Postgres indisponível... tente novamente em instantes") em vez de vazar a exceção crua do driver `pg`. Teste novo em `tools.test.ts`. **Não coberto nesta fatia** (escopo consciente, menor): a rota REST `routes/agenda.ts` e o job de despacho em loop `processDueAgenda` (ambos também tocam Postgres, mas não foram tratados — a tool MCP era o alvo literal do texto original do item). **RAG ingest é categoricamente diferente** dos outros — não existe "fallback markdown" conceitual (o propósito É popular o banco vetorial a partir do markdown já existente); hoje vaza a exceção crua do `pg` também, sem tratamento — candidato a "falhar rápido com mensagem clara" no mesmo espírito, não a um fallback de dados. | P1 | Fatia 1+2/N — REST/job de agenda e RAG ingest seguem abertos |
