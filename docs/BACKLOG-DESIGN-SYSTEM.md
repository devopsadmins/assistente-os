...
| DS2 | Notas operacionais do A1 nunca foram pro spec do B (bundler resolution, content glob, memoize de `brand`) | A1 → B | P1 | S | ✅ 2026-09-05 | — |
| DS3 | `Field` não compõe com `Select` (só documentado, não resolvido de verdade) | A2a → A2b | P1 | M | ✅ 2026-09-05 | — |
| DS4 | Testes de a11y faltando em Tabs/DropdownMenu/Switch/Checkbox (hoje verificados limpos, sem teste de regressão) | A2a | P2 | S | ✅ 2026-09-05 | — |
...
**Estado (✅ 2026-09-05)**: subseção "1.1. Notas de integração herdadas do A1" adicionada ao spec do B com os 3 pontos. Verificado no caminho: (a) `moduleResolution: "Bundler"` e (b) o glob de `content` do Tailwind já estavam corretos no scaffold real de `packages/web` — documentados como já-corretos, não como gap; (c) memoizar `brand` do `<ThemeProvider>` segue como nota real pra quando D (branding por conta) ligar `brand` de verdade (v1 usa `brand={null}`, uma constante, então o problema é inerte hoje).
**Aceitação**: ✅ subseção "Notas de integração herdadas do A1" adicionada ao spec do B com os 3 pontos.
...
**Estado (✅ 2026-09-05)**: `Field` ganhou uma segunda forma de `children` — função de render-prop `(render: FieldRenderProps) => ReactNode`, recebendo `{ labelId, controlId, describedBy, invalid }`. O caller compõe sua própria árvore (`<Select><SelectTrigger id={controlId} aria-labelledby={labelId} ...>`), sem depender de `cloneElement` alcançar um nó que não existe. `Label` ganhou `id={labelId}` pra sustentar o `aria-labelledby`. Verificado visualmente via Ladle real (Playwright): label clicável abre o Select, hint/erro renderizam corretamente, `aria-invalid`/`aria-describedby` chegam no `SelectTrigger`.
**Aceitação**: ✅ segunda forma de uso via render-prop. ✅ Stories `WithSelect`/`WithSelectError` em `field.stories.tsx`, verificadas visualmente.
**Arquivos**: `packages/ui/src/components/field.tsx` (+ `field.test.tsx`, `field.stories.tsx`).
...
**Estado (✅ 2026-09-05)**: um teste `expectNoA11yViolations` adicionado em cada um dos 4 arquivos de teste (para `DropdownMenu`, o menu é aberto antes de rodar o axe, já que o conteúdo vive num Portal só montado quando aberto).
...
**Estado (✅ 2026-09-05)**: `TINT_CHROMA_CAP = 0.05` aplicado nos índices 0-2 da rampa (via `Math.min`, nunca clampa um seed de baixo chroma pra cima). Snapshot real (`derive.test.ts`, seed `#3b82f6`) confirma o antes/depois: `--accent` caiu de chroma `0.1218` pra `0.05`, `--chat-user-bubble` de `0.1446` pra `0.05`. Verificado ao vivo via Ladle real (Playwright, preset "Cool" `#2b7fd8`): `getComputedStyle` confirma `--accent`/`--chat-user-bubble` em chroma `0.05` exatos, tint visivelmente sutil (antes seria um azul bem mais saturado).
**Aceitação**: ✅ cap de chroma adicional pros passos claros da rampa (índices 0-2), restaurando a intenção do spec.
**Arquivos**: `packages/ui/src/theme/ramp.ts` (+ `ramp.test.ts`, `derive.test.ts` + snapshot).
...
**Estado (✅ 2026-09-05)**: todo `color()` do preset agora gera `oklch(var(--x, <default>) / <alpha-value>)`, com o fallback igual ao valor de `:root` em `tokens.css` pra cada var. Teste novo (`tokens.test.ts`) faz o cross-check automático entre o preset e `tokens.css` — qualquer drift futuro entre os dois falha o teste, não só um review manual.
**Aceitação**: ✅ `oklch(var(--x, <default>) / <alpha-value>)` com fallback igual a `tokens.css` `:root`.
**Arquivos**: `packages/ui/tailwind-preset.cjs` (+ `src/tokens.test.ts`).

### DS7 — Limpeza cosmética em lote  ·  P3
Junta vários achados Minor das revisões (nenhum sozinho justifica uma tarefa). **Estado (✅ 2026-09-05, todos os 6 pontos verificados/corrigidos)**:
- ✅ `CardTitle` ganhou `asChild` (via `@radix-ui/react-slot`, mesmo padrão de `Button`) — permite `AuthCard`/`SettingsPanel` usarem outro nível de heading mantendo o estilo.
- ✅ `Toaster` ganhou props opcionais `swipeDirection`/`duration`, repassadas pro `ToastProvider` — antes eram hardcoded (`swipeDirection="right"` fixo, `duration` nem exposto).
- ✅ `DialogOverlay` e `ToastEntry` adicionados ao barrel (`index.ts`).
- ✅ Varredura de cor hardcoded (`no-hardcoded-color.test.ts`) agora recursa de verdade em subpastas, e passou a cobrir `src/hooks/`/`src/lib/` também (não só `src/components/` raiz). `src/theme/` fica deliberadamente fora — é a implementação do motor de cor (tabela de cores nomeadas, conversão OKLCH), onde literais de cor são o propósito, não um vazamento de estilo hardcoded.
- ✅ Verificado: `package.json` já estava em ordem alfabética em `dependencies`/`devDependencies` (checado programaticamente) — nada a corrigir, a nota original provavelmente antecedia uma limpeza já feita.
...
**Estado (✅ 2026-09-05)**: o arquivo referenciado originalmente (`docs/superpowers/specs/2026-09-01-design-system-packages-ui-design.md`) não contém esse texto — a Global Constraint de verdade vive em `docs/superpowers/plans/2026-09-01-design-system-primitives.md` (linha 16), o plano de implementação do A2a. Reformulada lá pra "todo componente **que renderiza seu próprio elemento DOM** é um `forwardRef`" — mesma redação que o plano seguinte (A2b-1, `2026-09-02-design-system-content-components.md:15`) já tinha adotado, corrigindo a assimetria entre os dois planos.
**Aceitação**: ✅ reformulado.
**Arquivos**: `docs/superpowers/plans/2026-09-01-design-system-primitives.md`.
...
| 2026-09-05 | DS2 | Concluído — subseção "Notas de integração herdadas do A1" adicionada ao spec do B. |
| 2026-09-05 | DS3 | Concluído — `Field` ganhou forma render-prop pra compor com `Select` sem `cloneElement`; verificado visualmente via Ladle. |
| 2026-09-05 | DS4 | Concluído — 4 testes `expectNoA11yViolations` novos (Tabs/DropdownMenu/Switch/Checkbox). |
| 2026-09-05 | DS5 | Concluído — `TINT_CHROMA_CAP` nos índices 0-2 da rampa; verificado com snapshot real e ao vivo via Ladle. |
| 2026-09-05 | DS6 | Concluído — fallback em todo `color()` do preset, com teste de drift contra `tokens.css`. |
| 2026-09-05 | DS7 | Concluído — os 6 achados resolvidos (CardTitle asChild, Toaster props, barrel exports, sweep recursivo, package.json já ok, Markdown/Citation forwardRef). |
| 2026-09-05 | DS9 | Concluído — Global Constraint reformulada no plano correto (`design-system-primitives.md`, não o spec originalmente referenciado). |
