...
| **Onda 3d** — centralizar config | **Fase 1 concluída 2026-09-04:** 5 fixes (2 bugs reais + 3 riscos de duplicação unificados). Ver [plano](superpowers/plans/2026-09-04-onda-3d-centralizar-config-fase1.md) e seção "Fora de escopo" nele para o que fica para Fase 2. ~58 `process.env` soltos restantes → `loadConfig` + schema (zod/envalid); matar pares de alias (`ADO_*`/`AZURE_DEVOPS_*`). Refactor grande em fases. |
| **Onda 3e** — quebrar god-objects | **Fase 1 concluída 2026-09-04:** (a) `handleChat` (chat.ts): extraído em 3 sub-handlers (`handleChatBuffer`, `handlePostChat`, `handleChatLanggraphStatus`) + pipeline de prompt compartilhado em `promptPipeline.ts`, em módulos dedicados; `chat.ts` agora é um dispatcher fino (22 l.). Ver [plano](superpowers/plans/2026-09-04-onda-3e-split-chat-handler.md). (b) `tools/src/index.ts`: 4 famílias migradas (`guardian_*` 7 + `browser_*` 8 + `ado_*` 10 + `worktree_*`/`mission_*` 6 = 31 tools no total), arquivo reduzido de 2075 para 1262 linhas, padrão `ToolContext`/`FAMILY_HANDLERS` estabelecido. Ver [plano](superpowers/plans/2026-09-04-onda-3e-split-tools-dispatcher.md). Fase 2 em ambos os itens (17 variáveis de `handlePostChat` + 8 famílias restantes listadas na seção "Fora de escopo" do plano de tools) fica como trabalho futuro opcional, não perdido. |
