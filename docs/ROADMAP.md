...
| **Onda 3e** — quebrar god-objects | **Fase 1 concluída 2026-09-04:** `handleChat` extraído em 3 sub-handlers (`handleGetChatHistory`, `handlePostChat`, `handleStreamChat`) em módulos dedicados; `chat.ts` agora é um dispatcher fino (22 l.). Fase 2 (decompor as 17 variáveis compartilhadas de `handlePostChat` em fases explícitas com um objeto de contexto) fica como trabalho futuro opcional, não perdido. `tools/src/index.ts` (1859 l.) ainda pendente. |
