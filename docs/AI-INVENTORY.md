...
| 10 | **Roteador local-first** | `core/router.ts` | Escolher o degrau (local/zen/soul) por sonda de disponibilidade | Config + resultado da sonda (sem conteúdo do prompt) | **AI-1** | Não aplica modelo ao conteúdo; só decide rota; toda tentativa em `router_history` imutável | Téc.: agente assistente-os · Risco: security-reviewer — aceito 2026-08-27 |
