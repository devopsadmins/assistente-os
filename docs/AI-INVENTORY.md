...
| 11 | **Vertical Clínicas** (soul `clinica_template`, tools `clinic_triage_lead`/`clinic_prevent_noshow` — ainda não escalada) | `packages/tools/src/clinic/` (planejado), `~/.assistant-os/souls/clinica_template/` (planejado) | Triagem de lead + prevenção de no-show para clínicas (odontologia, estética avançada) | Score/dossiê de lead, procedimento médico, agenda de paciente | **AI-4** (confirmado via `standards_classify_profile` — dado de saúde em runtime) | **Nenhum ainda em produção** — `standards_gate_blockg` retornou BLOCKED (G3/G4/G5 reprovados); nenhum dado real de paciente deve fluir até o Bloco G ser aprovado. Ver `docs/adr/ADR-PRIV-003-clinicas-base-legal-retencao-ai4.md` e `docs/compliance/clinicas/` | Téc.: agente terrasIA · Neg.: produto SLC Clínicas (owner a confirmar) · Risco: security-reviewer / DPO — a nomear — **Proposta 2026-09-07**, não aceito |
