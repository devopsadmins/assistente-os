...
| 11 | **Vertical Clínicas** (soul `clinica_template`, tools `clinic_triage_lead`/`clinic_prevent_noshow`) | `packages/tools/src/clinic/index.ts`, `~/.assistant-os/souls/clinica_template/` | Triagem de lead + prevenção de no-show para clínicas (odontologia, estética avançada) | Score/dossiê de lead, procedimento médico, agenda de paciente | **AI-4** (confirmado via `standards_classify_profile` — dado de saúde em runtime) | Scaffolding técnico existe (Fase 2, modo dry-run) — `clinic_triage_lead` é `L2`, `clinic_prevent_noshow` é `L3` no catálogo (`policy.ts`) e sempre roda em dry-run (nunca dispara mensagem real); soul com `autonomy: "ask"` + `approvalPolicy` cobrindo `clinic_prevent_noshow`. **Nenhum dado real de paciente em produção** — `standards_gate_blockg` retornou BLOCKED (G3/G4/G5 reprovados); só avança pra produção quando o Bloco G for aprovado. Ver `docs/adr/ADR-PRIV-003-clinicas-base-legal-retencao-ai4.md` e `docs/compliance/clinicas/` | Téc.: agente terrasIA · Neg.: produto SLC Clínicas (owner a confirmar) · Risco: security-reviewer / DPO — a nomear — **Proposta 2026-09-07**, não aceito |
