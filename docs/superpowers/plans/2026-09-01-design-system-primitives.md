...
- Every component **that renders its own DOM element** is a `React.forwardRef` accepting `className` and merging it last via `cn()` from `packages/ui/src/lib/cn.ts` (`import { cn } from "../lib/cn"`), so consumers can always override/extend styling. (DS9, `docs/BACKLOG-DESIGN-SYSTEM.md`: the unqualified original wording was too broad — root re-exports of Radix context providers with no DOM node of their own, like `Dialog`/`Popover`/`Select`/`DropdownMenu`/`Tooltip`/`Tabs`, can't honestly forward a ref to something that doesn't exist. The A2b-1 plan already adopted this qualified wording; this fixes the earlier A2a plan to match.)
