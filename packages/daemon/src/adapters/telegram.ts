/**
 * Adaptador Telegram - Human-in-the-Loop
 *
 * Recebe payloads JSON do Telegram Bot API, resolve a Alma ativa,
...
// ── Processar payload Telegram ────────────────────────────────────────

async function processTelegramPayload(
  payload: any,
  ...
  try {
    draft = await ollamaChat(
      cfg,
      payload.data?.body || "Mensagem de voz/mídia recebida",
      ragContext
    );
  } catch (err) {
    // Ollama offline → fallback
    draft = `🤖 Mensagem recebida de ${payload.from}:\n> ${payload.data?.body || "[sem texto]"}\n\n*Resposta automática offline - Ollama não disponível.*`;
  }
  ...
  try {
    ...
    __annotar(soulDirPath, `Telegram [${payload.from}]: ${payload.data?.body || "[sem texto]"}`, sessionDate);
  } catch (err) {
...
export function registerTelegramRoutes(_app: any) {
  // Lógica real nos endpoints HTTP no server.ts
}

if (require.main === module) {
  console.log("Telegram adapter loaded - ready for HTTP integration");
}

export { processTelegramPayload };